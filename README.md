...
## Quickstart — 8 steps from zero to running
...
### Step 4 — Make a copy of the settings file
...
### Step 6 — Download the project's libraries
...
The wallets dashboard appears immediately. That's it — you're done.
...
**Why bother with the middle server?** If we put your API secret directly in the browser code, anyone visiting the site could open the developer tools and copy it. Keeping it server-side keeps you safe — even if you later put this app on the public internet.
...
| The wallets page is empty | Your organization has no wallets yet — not an error. | Create some wallets in the Lumen dashboard, then refresh. |
...
> **CORS** (Cross-Origin Resource Sharing) is a browser security rule that blocks websites on one address from talking to a server on a different address — unless the server explicitly allows it.
